...
(function() {
    ...
    // Detect page type from URL path so no inline initializePage() call is needed.
    function detectPageType() {
        var path = window.location.pathname.toLowerCase();
        if (path.includes('reset')) return 'resetpassword';
        if (path.includes('forgot')) return 'forgotpassword';
        if (path.includes('signup') || path.includes('sign-up')) return 'signup';
        if (path.includes('signin') || path.includes('sign-in') || path.includes('login')) return 'signin';
        // Fallback: parse from document title
        var title = document.title.toLowerCase();
        if (title.includes('reset')) return 'resetpassword';
        if (title.includes('forgot')) return 'forgotpassword';
        if (title.includes('sign up')) return 'signup';
        return 'signin';
    }
    ...
    function initialize(pageType) {
        ...
        if (pageType) state.pageType = pageType;
        log('Initializing ' + state.pageType + ' page');
        waitForApiContainer();
        trackPageView(state.pageType);
        addCustomEventListeners();
        handleUrlParameters();
    }

    // Keep window.initializePage for any existing callers (e.g. signup.html).
    window.initializePage = initialize;
    ...
    function onFormInjected() {
        ...
        // On the reset-password page B2C injects a "Change e-mail" button that
        // lets users switch email mid-flow — not needed for the Facilon set-password
        // invitation flow where the email is already known and fixed.
        if (state.pageType === 'resetpassword') {
            var changeEmailBtn = document.getElementById('email_ver_but_edit');
            if (changeEmailBtn) {
                changeEmailBtn.style.display = 'none';
                log('Hid #email_ver_but_edit (Change e-mail button)');
            }
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        log('Page type detected: ' + state.pageType);
        log('Browser Info:', getBrowserInfo());
        checkRequirements();

        // Auto-start — no inline initializePage() call required in HTML.
        initialize();
    });
